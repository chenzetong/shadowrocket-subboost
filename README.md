...
3. 进入配置页，将导入的配置设为使用中。
4. 在 `🚀 节点选择 1` 和 `🚀 节点选择 2` 中分别选择需要的订阅节点。
...
- `PROXY` 是 Shadowrocket 的内置全局代理策略，使用 App 当前选中的代理节点；多个策略组选择 `PROXY` 时会共享同一个出口。
- `🚀 节点选择 1` 和 `🚀 节点选择 2` 通过 `policy-regex-filter=.*` 动态加载订阅中的全部节点，并分别记忆选择，因此可以使用不同出口。
- AI、Gemini、Google 默认引用 `🚀 节点选择 1`。
- 券商、PayPal/Stripe/Wise、Binance/Bybit 默认引用 `🚀 节点选择 2`。
- 原 `🚀 节点选择` 继续保留，作为其他规则和旧配置的兼容入口。
